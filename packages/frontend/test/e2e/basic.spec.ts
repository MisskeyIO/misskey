...
test.describe('After setup instance', () => {
	...
	test('signup', async ({ page }) => {
		await page.route('https://api.pwnedpasswords.com/range/*', route => route.fulfill({ body: '' }));
		await visitHome(page);
...
test.describe('After user setup', () => {
	...
	test('リアルタイム更新を無効化できない', async ({ page }) => {
		await test.expect(page.getByTestId('open-post-form')).toBeVisible();
		await page.evaluate(() => {
			const key = 'idbfallback::pizzax::base';
			const state = JSON.parse(window.localStorage.getItem(key) ?? '{}');
			window.localStorage.setItem(key, JSON.stringify({ ...state, realtimeMode: false }));
		});
		const messages: Array<{ type?: string; body?: { channel?: string; params?: { minimize?: boolean; }; }; }> = [];
		...
		await page.reload();
		await test.expect.poll(() => messages.some(message => message.type === 'connect'
			&& message.body?.channel === 'homeTimeline' && message.body.params?.minimize === true)).toBe(true);
	});
