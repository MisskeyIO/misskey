...
function dependencies(idempotent: string | null = null) {
	const draft = { id: 'draftid', userId: 'userid' };
	const redis = {
		get: jest.fn(async (_key: string) => idempotent),
		set: jest.fn(async (_key: string, _value: string, _mode: string, _ttl: number) => 'OK'),
	...
	const noteDraftService = {
		...
		create: jest.fn(async (_me: unknown, _data: unknown) => draft),
	};
...
describe('notes/createの予約互換', () => {
	test('将来時刻は即時投稿せずserver draftへ保存する', async () => {
		...
		expect(target.redis.set).toHaveBeenLastCalledWith(expect.any(String), target.draft.id, 'EX', 60);
	});
