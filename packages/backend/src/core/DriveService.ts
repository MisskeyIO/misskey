...
export class DriveService {
	...
	public async addFile({
		...
		if (user != null && !isLink) {
			...
			if (!isModerator) {
				const userPolicies = policies ?? await this.roleService.getUserPolicies(user.id);

				const allowedMimeTypes = userPolicies.uploadableFileTypes;
				...
				const driveCapacity = 1024 * 1024 * userPolicies.driveCapacityMb;
				const maxFileSize = 1024 * 1024 * userPolicies.maxFileSizeMb;
