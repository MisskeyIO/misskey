...
export class UserFollowingService implements OnModuleInit {
	...
	public async follow(
		...
		if (
			...
			(this.userEntityService.isLocalUser(followee) && this.userEntityService.isRemoteUser(follower) && this.utilityService.isItemListedIn(follower.host, this.meta.silencedHosts))
		) {
