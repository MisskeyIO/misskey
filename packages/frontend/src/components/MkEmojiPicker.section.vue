...
<section v-else>
  <header class="_acrylic" @click="shown = !shown">
    <i class="toggle ti-fw" :class="shown ? 'ti ti-chevron-down' : 'ti ti-chevron-up'"></i> <slot></slot> (<i class="ti ti-folder ti-fw"></i>:{{ customEmojiTree.length }} <i class="ti ti-fw"></i>:{{ emojis.length }})
  </header>
  <div v-if="shown" class="body">
    <button
        v-for="emoji in emojis"
