...
export class FanoutTimelineService {
	...
	@bindThis
	public async purgeWithDimensions(name: FanoutTimelineName): Promise<void> {
		const baseKey = 'list:' + name;
		await this.redisForTimelines.del(baseKey);

		let cursor = '0';
		do {
			const [nextCursor, keys] = await this.redisForTimelines.scan(cursor, 'MATCH', `${baseKey}:*`, 'COUNT', 100);
			cursor = nextCursor;
			if (keys.length > 0) {
				await this.redisForTimelines.del(...keys);
			}
		} while (cursor !== '0');
	}
