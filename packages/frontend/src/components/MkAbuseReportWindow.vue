...
<MkWindow v-if="page === 1" ref="uiWindow" :initialWidth="500" :initialHeight="500" :canResize="true" @closed="emit('closed')">
	...
	<div class="_spacer" style="--MI_SPACER-min: 20px; --MI_SPACER-max: 28px;">
		<div class="_gaps_m" :class="$style.root">
			...
			<MkButton primary full :disabled="comment.length === 0 || category === ''" @click="send">{{ i18n.ts.send }}</MkButton>
		</div>
	</div>
</MkWindow>
...
type ApiCategory = NonNullable<Misskey.Endpoints['users/report-abuse']['req']['category']>;
type ReportCategory = '' | 'notLike' | ApiCategory;
...
] satisfies MkSelectItem<ReportCategory>[];
