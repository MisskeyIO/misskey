...
>
			...
			<div :class="[$style.contentWrapper, { [$style.hideForFallback]: hideForFallback }]">
				...
				>
					<div :class="$style.hiddenWrapper">
						<MkBlurhash
							v-if="content.file?.blurhash != null"
						...
						/>
						<img
							v-else-if="content.type === 'video' && content.thumbnailUrl != null && !content.file?.isSensitive"
	...
	<div :class="[$style.footer, { [$style.infoShowing]: infoShowing && !isZooming }]">
		<div v-if="content.type === 'video' && !blocked && !hide && !prefer.s.useNativeUiForVideoAudioPlayer" :class="$style.mediaControl">
...
function shouldHideInGallery(content: Content): boolean {
	...
	// 最初の画像や隣の画像でも、同意前は表示しない。
	if (content.file.isSensitive && sensitiveContentConsent.value !== true) return true;
	const hiddenByDefault = shouldHideFileByDefault(content.file, true);
	if (!hiddenByDefault) return false;

	// 表示を承認済みの画像だけ、再確認せずに開く。
	if (content.file.isSensitive && content.initiallyRevealed) {
		return false;
	}
...
const blocked = computed(() => props.content.file != null && isFileBlocked(props.content.file));

watch(() => props.content, () => {
	hide.value = shouldHideInGallery(props.content);
}, { deep: true, immediate: true });

watch(sensitiveContentConsent, () => {
	if (props.content.file?.isSensitive && sensitiveContentConsent.value !== true) hide.value = true;
});
