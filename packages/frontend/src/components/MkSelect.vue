...
function show() {
	if (!inputEl.value || opening.value || props.disabled || props.readonly) return;
	focus();
			...
			if (item.type === 'group') {
				...
				for (const option of item.items) {
					menu.push({
						...
						action: () => {
							emit('update:modelValue', option.value as ValuesOfItems<T>);
						},
					});
				}
			} else {
				menu.push({
					...
					action: () => {
						emit('update:modelValue', item.value as ValuesOfItems<T>);
					},
				});
			}
		}
