			...
			<div class="_gaps_s">
				...
				<MkFolder v-if="iAmAdult" :defaultOpen="true">
					...
					<div class="_gaps_s">
						...
						<MkSelect v-model="draft.displayOfSensitiveAds" :items="sensitiveAdsItems">
...
const nsfwItems = [{
	label: i18n.ts._displayOfSensitiveMedia.respect,
	value: 'respect',
}, {
	label: i18n.ts._displayOfSensitiveMedia.ignore,
	value: 'ignore',
}, {
	label: i18n.ts._displayOfSensitiveMedia.force,
	value: 'force',
}] satisfies MkSelectItem[];

const sensitiveAdsItems = [{
	label: i18n.ts._displayOfSensitiveAds.hidden,
	value: 'hidden',
}, {
	label: i18n.ts._displayOfSensitiveAds.always,
	value: 'always',
}, {
	label: i18n.ts._displayOfSensitiveAds.filtered,
	value: 'filtered',
}] satisfies MkSelectItem[];
...
const decoIcons = computed(() => {
	...
	return DECO_CONFIG.items.map((deco, index) => {
		...
		const tint = Object.hasOwn(DECO_CONFIG.tints[mode], icon) ? DECO_CONFIG.tints[mode][icon as keyof typeof DECO_CONFIG.tints[typeof mode]] : fallbackTint;
