...
const blocked = computed(() => isFileBlocked(props.video));

watch(() => props.video, () => {
	hide.value = shouldHideFileByDefault(props.video);
}, { deep: true });

watch(sensitiveContentConsent, () => {
	if (props.video.isSensitive && sensitiveContentConsent.value !== true) hide.value = true;
});
