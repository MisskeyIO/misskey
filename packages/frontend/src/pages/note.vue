...
function fetchNote() {
	...
	misskeyApi('notes/show', {
		...
		note.value = res;
		const fileIds = res.fileIds ?? [];
		if (res.userId === $i?.id && fileIds.length > 0) {
			const noteId = res.id;
			Promise.all(fileIds.map(fileId => misskeyApi('drive/files/show', { fileId }))).then(files => {
