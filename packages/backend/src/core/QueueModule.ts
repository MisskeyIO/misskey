...
const $postScheduledNote: Provider = {
	...
	useFactory: (config: Config) => new Bull.Queue(QUEUE.POST_SCHEDULED_NOTE, baseQueueOptions(config.redisForSystemQueue, config.bullmqQueueOptions, QUEUE.POST_SCHEDULED_NOTE)),
	inject: [DI.config],
};
...
const $systemWebhookDeliver: Provider = {
	...
	useFactory: (config: Config) => new Bull.Queue(QUEUE.SYSTEM_WEBHOOK_DELIVER, baseQueueOptions(config.redisForWebhookDeliverQueue, config.bullmqQueueOptions, QUEUE.SYSTEM_WEBHOOK_DELIVER)),
	inject: [DI.config],
};
