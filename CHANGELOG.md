...
- Fix: メールアドレスの登録・認証が必要な場合の案内を復元
- Fix: 移行前の予約投稿が失敗した場合に内容と理由を表示
- Fix: 解析への同意を保存済みの場合にクライアントの起動が失敗する問題を修正
- Fix: 絵文字管理・設定・ギャラリー画面が読み込みに失敗する問題を修正
- Enhance: 新しい画像ビューアーでもioの表示確認と音声設定を維持
- Fix: 2025.4の通信節約設定を端末・サーバー同期の両方から引き継ぐ
- Fix: 投稿欄を消去した後も公開範囲と連合設定を保持する
- Fix: キャッシュ削除の直列待ちと絵文字の重複取得を減らし、失敗時も操作へ戻れるよう修正
- Fix: 投稿フォームを閉じて開き直したとき、書きかけと投稿設定を復元する
- Fix: キャッシュ削除を時間で中断せず、更新処理の完了を待つよう修正
- Enhance: キャッシュ削除中の絵文字再取得を省き、リロード後の通常取得へ任せる

### Server
- Fix: フォロー解除時に件数の更新完了を待たず応答する問題を修正
