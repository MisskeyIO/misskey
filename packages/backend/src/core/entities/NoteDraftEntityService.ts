...
export class NoteDraftEntityService implements OnModuleInit {
	...
	@bindThis
	public async packAttachedFiles(fileIds: MiNote['fileIds'], packedFiles: Map<MiNote['fileIds'][number], Packed<'DriveFile'> | null>, me?: { id: MiUser['id'] } | null): Promise<Packed<'DriveFile'>[]> {
		const missingIds = [];
