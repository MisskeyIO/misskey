...
describe('SearchService', () => {
	...
	describe('opensearch', () => {
		...
		test('日時範囲とページ位置をOpenSearchにも渡す', async () => {
			const start = 0;
			const end = Date.now();
			const since = ctx.idService.gen(end - 2000);
			const until = ctx.idService.gen(end - 1000);
			search.mockResolvedValue({ body: { hits: { hits: [] } } });

			await ctx.service.searchNote('hello', null, { rangeStartAt: start, rangeEndAt: end }, { limit: 10, sinceId: since, untilId: until });
			expect(search.mock.calls[0][0].body.query.bool.must).toEqual(expect.arrayContaining([
				{ range: { createdAt: { gte: start } } },
				{ range: { createdAt: { lte: end } } },
				{ range: { createdAt: { gt: ctx.idService.parse(since).date.getTime() } } },
				{ range: { createdAt: { lt: ctx.idService.parse(until).date.getTime() } } },
			]));
		});
