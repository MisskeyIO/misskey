...
<div v-else-if="muted && !hideByPlugin" class="_panel" :class="$style.muted" @click="muted = false">
...
const reactionTabTypeTrimLocal = computed(() => reactionTabType.value?.replace('@.', '') ?? '');
...
function reactionMuteToggle(reactionName: string): void {
